...
- [~] Backup DB + sinh migration cho 7 model thiếu (`Task`, `AiConfig`, `MasterPart`, `Supplier`, `CiplRecord`, `CiplItem`, `TaskStatus`) — **soạn xong 22/09, CHƯA áp lên DB thật** (chờ người dùng đồng ý + tự backup, theo đúng cảnh báo "đừng ghi vào DB dùng chung khi chưa được cho phép rõ ràng" ở CLAUDE.md): `prisma/migrations/20260922130000_missing_models/migration.sql`, hand-written/idempotent giống phong cách CBU migration — `CREATE TABLE IF NOT EXISTS` cho 6 bảng, `CREATE TYPE`/`ADD CONSTRAINT` bọc `DO $$ … EXCEPTION WHEN duplicate_object` (Postgres không có `IF NOT EXISTS` cho hai lệnh này), cột lấy đúng từ `npx prisma migrate diff --from-empty --to-schema prisma/schema.prisma --script` (không gõ tay). Kiểm chứng bằng `node scripts/verify-missing-models-migration.mjs` (Postgres nhúng, 20 kiểm tra): áp trên nền `init` (tạo mới đủ 6 bảng + enum, FK hoạt động đúng kể cả cascade delete CiplRecord→CiplItem, FK chặn giá trị sai); áp trên DB đã có sẵn bảng `Supplier` khác cấu trúc (mô phỏng đúng tình huống production — bảng được tạo ngoài luồng migration từ trước) để chứng minh migration bỏ qua hoàn toàn, không đụng dữ liệu/cấu trúc đã có; và áp cả 4 migration theo đúng thứ tự thời gian không xung đột. **Việc còn lại trước khi áp thật:** xác nhận cột hiện có trên Supabase khớp `schema.prisma` (migration này im lặng bỏ qua nếu bảng đã tồn tại, không tự sửa lệch), rồi backup → áp → `prisma migrate resolve --applied`
