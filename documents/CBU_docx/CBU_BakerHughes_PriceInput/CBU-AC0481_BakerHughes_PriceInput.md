# CBU — AC0481 Baker Hughes (Malaysia) — PRICE INPUT MODE (nhập giá bán)
...
> ⚠️ **File này KHÔNG có Named Ranges** — mọi công thức dùng tham chiếu ô trực tiếp (`N6`, `$N$7`...). Markdown dưới đây tự đặt TÊN BIẾN gợi ý trong phần công thức để dễ đọc, nhưng bản thân file Excel vẫn dùng địa chỉ ô thô.

> ⚠️ **Khác biệt cốt lõi so với bản MARGIN INPUT:** cột **Sales Price (G)** đổi từ ⚙️ COMPUTED (`=ROUNDUP(UnitCost/(1-MarginPct),0)`) → 🟦 **INPUT** (nhập tay trực tiếp mỗi dòng hàng, cả khối FCA lẫn DAP); cột **% Margin (J)** đổi từ 🟦 INPUT → ⚙️ COMPUTED (`=IF(SalesPrice=0,0,MarginPerUnit/SalesPrice)`), tính NGƯỢC sau khi đã có giá bán. Margin per unit (I) vẫn luôn là công thức `SalesPrice − UnitCost` như cũ, không đổi.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 480131200 | 1R MODEL F STD. SERVICE DRILL PIPE FLOAT VALVE | KEYSTONE | 30 | 0.6624 | 131 | 3,930 | 22.67 | 17.3% | 680 | 3,250 | 108.33 | 105.50 | 2.83 |
|  |  |  |  | **30.00** | **19.87** | **3,930.00** | **3,930.00** | **680.00** | **17.3%** | **680.00** | **3,250.00** | **3,250.00** | **3,165.00** | **85.00** |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 480131200 | 1R MODEL F STD. SERVICE DRILL PIPE FLOAT VALVE | KEYSTONE | 30 | 0.6624 | 131 | 3,930 | 22.67 | 17.3% | 680 | 3,250 | 108.33 | 105.50 | 2.83 | 60.00 |
|  |  |  |  | **30.00** | **19.87** | **3,930.00** | **3,930.00** | **680.00** | **17.3%** | **680.00** | **3,250.00** | **3,250.00** | **3,165.00** | **85.00** | **1,800.00** |
...
| ITEM | Part No. | Description | Supplier | Q'ty | Weight (kg) | Sales Price | Total Revenue | Margin per unit | % Margin | Total Margin | Total Cost | Unit Cost | Material Cost | Financial Cost | Freight per Logistic (reference) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 480131200 | 1R MODEL F STD. SERVICE DRILL PIPE FLOAT VALVE | KEYSTONE | 30 | 0.6624 | 133 | 3,990 | 22.69 | 17.1% | 680.6562 | 3,309.3438 | 110.31 | 105.50 | 4.81 | 60.00 |
|  |  |  |  | **30.00** | **19.87** | **3,990.00** | **3,990.00** | **680.66** | **17.1%** | **680.66** | **3,309.34** | **3,309.34** | **3,165.00** | **144.34** | **1,800.00** |
...
## 6. Khác biệt so với bản MARGIN INPUT

| Hạng mục | Bản MARGIN INPUT | Bản PRICE INPUT (file này) |
|---|---|---|
| Cột **G — Sales Price** | ⚙️ COMPUTED — `ROUNDUP(UnitCost/(1-MarginPct),0)` | 🟦 **INPUT** — nhập tay trực tiếp từng dòng, cả FCA lẫn DAP |
| Cột **J — % Margin** | 🟦 INPUT — mục tiêu margin gõ tay | ⚙️ COMPUTED — `IF(G=0,0,I/G)`, tính ngược từ giá đã nhập |
| Cột **I — Margin per unit** | ⚙️ COMPUTED — không đổi | ⚙️ COMPUTED — không đổi (vẫn = SalesPrice − UnitCost) |

## 7. Tổng hợp các điểm cần lưu ý (rủi ro kỹ thuật kế thừa từ bản Margin Input)
