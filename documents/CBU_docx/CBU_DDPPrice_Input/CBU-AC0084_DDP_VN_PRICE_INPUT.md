# CBU — DDP VN (Hoàng Sơn) — PRICE INPUT MODE (nhập giá bán)

> Sheet đích tính giá: **Margin Analysis** (kèm **Logistic**, **Bank Fee** làm nguồn dữ liệu phụ trợ). File này dùng chế độ **NHẬP TRỰC TIẾP DDP PRICE (giá bán) CHO TỪNG DÒNG HÀNG → HỆ THỐNG TỰ TÍNH NGƯỢC % MARGIN & MARGIN PER UNIT**. Đây là chiều ngược lại của bản MARGIN INPUT (nhập % margin mục tiêu → tính ra giá bán).

**Chú giải nhóm biến:**

| Nhóm | Ý nghĩa |
|---|---|
| 🗄️ **DATABASE** | Dữ liệu lấy sẵn từ database web app (Part No., Description, Supplier, Qty, Total Weight, Material Cost, Customer, Country, Goods origin, Incoterm, Payment terms, Inquiry date...) |
| 🟦 **INPUT** | Người dùng gõ tay trực tiếp (Job No., %Duty, Forwarder, cước phí, Base amount, và ĐẶC BIỆT là **DDP Price (USD)** — giá bán cho khách) |
| 🟧 **DEFAULT** | Tham số/chính sách có sẵn giá trị chuẩn, ít đổi nhưng có thể ghi đè |
| 🟧 **DEFAULT\*** | Giống DEFAULT nhưng bạn đã đánh dấu "(Bỏ)" — ứng viên cân nhắc loại bỏ, hiện vẫn còn trong công thức |
| ⚙️ **COMPUTED** | Công thức tính trong cùng sheet — không sửa tay |
| 🟩 **LINK** | Công thức lấy dữ liệu từ sheet khác — không sửa tay |

> ⚠️ **Khác biệt cốt lõi so với bản MARGIN INPUT:** cột **DDP Price (USD)** đổi từ ⚙️ COMPUTED → 🟦 **INPUT**; cột **% Margin** và **Margin per unit** vẫn luôn là ⚙️ COMPUTED nhưng giờ tính NGƯỢC (Price → Cost → Margin) thay vì XUÔI (Margin → Price). 2 cột **Margin % override / Margin $/unit override** và named range mặc định `TargetMarginPct` không còn được dùng trong công thức (xem mục 9).
...
| Target margin (m) | 🟧 DEFAULT* | 25.00% | Lãi gộp trên giá bán  ⚠️ KHÔNG còn dùng trong công thức ở bản này (giữ lại chỉ tham chiếu). |
...
**Named Ranges tương ứng:** `ExchangeRate_Quote`(F3) · `TargetMarginPct`(F4, *không còn dùng*) · `CommissionRatePct`(F5) · `CITRatePct`(F6) · `PercentValueFinanced`(F7) · `InterestRatePct`(F8) · `FinancingDays`(F9) · `DaysPerYear`(F10) · `VNDRoundingStep`(F11) · `LbToKgFactor`(F12)

## 3. BLOCK 1 — AIR FREIGHT (dòng 15–31)

| ITEM | Part No. | Description | Supplier | Q'ty | Total Weight (lb) | Weight (kg) | Material Cost | %Duty | Bank fee | Logistics | Duty | Commission | CIT | Unit Cost | DDP Price (USD) | DDP Price (VND) | Total Revenue (VND) | Margin per unit | % Margin | Total Margin | Total Cost |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
> Lưu ý: cột **G (Weight kg)** tuy nhãn ghi "(Database)" nhưng thực chất là **⚙️ CÔNG THỨC** `=TotalWeightLb ÷ Qty × LbToKgFactor` tính tự động từ 2 cột DATABASE thật (E, F).

Bản NHẬP GIÁ BÁN: cột DDP Price (USD) là Ô NHẬP TAY trực tiếp cho từng dòng hàng (giá báo cho khách). Margin per unit và % Margin KHÔNG cần nhập — hệ thống tự tính NGƯỢC LẠI từ Unit Cost và giá bán đã nhập.
...
# ⚠️ KHÁC BIỆT CHÍNH: DDPPriceUsd là INPUT trực tiếp, KHÔNG phải công thức
DDPPriceUsd     = <người dùng nhập tay — giá bán USD/đơn vị cho khách>

Commission      = CommissionRatePct × DDPPriceUsd     # vẫn phụ thuộc giá bán đã nhập
...
MarginPerUnit   = DDPPriceUsd − UnitCost              # KẾT QUẢ, tính ngược
MarginPctActual = MarginPerUnit ÷ DDPPriceUsd         # KẾT QUẢ, tính ngược
...
> Hai dòng CHECK phải luôn = 0. Nếu khác 0 nghĩa là công thức bị lệch.
...
| Margin Analysis | DDPPriceUsd (cột P) | DDP Price (USD) | INPUT | DDPPriceUsd giờ là Ô NHẬP TAY trực tiếp cho từng dòng hàng (giá báo cho khách), USD mỗi đơn vị. KHÔNG còn tính từ Target Margin / Override — chính là input, mọi biến khác (Commission, Margin per unit, % Margin) tính XUÔI hoặc NGƯỢC từ giá trị này. |
...
| Margin Analysis | MarginPctActual (cột T) | % Margin | COMPUTED | MarginPctActual = MarginPerUnit ÷ DDPPriceUsd<br><br>Tính NGƯỢC LẠI sau khi đã có DDP Price (USD) nhập tay — không còn liên quan Target Margin. |
...
## 9. Khác biệt so với bản MARGIN INPUT

| Hạng mục | Bản MARGIN INPUT | Bản PRICE INPUT (file này) |
|---|---|---|
| Cột **P — DDP Price (USD)** | ⚙️ COMPUTED — tính từ TargetMarginPct hoặc override | 🟦 **INPUT** — nhập tay trực tiếp cho từng dòng |
| Cột **T — % Margin** | ⚙️ COMPUTED — kết quả sau khi tính giá | ⚙️ COMPUTED — vẫn là kết quả, nhưng tính NGƯỢC từ giá đã nhập |
| Cột **W/X — Margin override** | Có (2 cột, tùy chọn) | **Đã xóa** — không còn ý nghĩa khi giá là input trực tiếp |
| `TargetMarginPct` (F4) | Dùng làm margin mặc định cho cả đơn | Vẫn còn trong file (DEFAULT) nhưng **không dùng trong công thức nào** |
| Các tham số bạn đánh dấu "(Bỏ)" | Target margin, % Value financed, Interest rate, Financing days | Giống bản MARGIN INPUT — vẫn còn trong công thức (trừ Target margin nay đã hết tác dụng thực tế), markdown chỉ phản ánh hiện trạng, chưa tự xóa gì |
