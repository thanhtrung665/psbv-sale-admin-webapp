...
export default function LegacyCbuCalcPage({ 
  params,
  searchParams
}: { 
  params: { id: string },
  ...
  // Next.js App Router passes searchParams directly to page components
  const cbuTypeParam = searchParams?.type;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        let initialMode = rawData.cbuMode || "MARGIN_INPUT";
        if (!rawData.cbuMode && cbuTypeParam === "price") {
          initialMode = "PRICE_INPUT";
        }
        ...
        const initInputs: Record<string, string> = {
          ...
          // G. Chi phí vốn & Pricing Parameters
          ...
          // H. Pricing Parameters (Global)
          targetMarginPercent: String(safeNum(rawData.targetMarginPercent, 25)),
          commissionRate: String(safeNum(rawData.commissionRate, 3)),
          citOnCommission: String(safeNum(rawData.citOnCommission, 20)),
        };
        ...
        const loadedItems: CBUItemEngineData[] = rawItems.map((item: any) => {
          ...
          initInputs[`${id}_marginPercent`] = item.marginPercent !== null && item.marginPercent !== undefined ? String(item.marginPercent) : "";
          initInputs[`${id}_marginOverrideUsd`] = String(safeNum(item.marginOverrideUsd, 0));
          initInputs[`${id}_targetDdpPriceUsd`] = String(safeNum(item.ddpPriceUsd, 0));
  ...
  useEffect(() => {
    if (!loading) {
      try {
        setRenderError("");
        // Commission & CIT are now global — inject into every item
        const globalCommissionRate = parseNumInput(inputs.commissionRate);
        const globalCitOnCommission = parseNumInput(inputs.citOnCommission);
        ...
        const engineItems = items.map(item => ({
          ...
          commissionPercent: globalCommissionRate,
          citPercent: globalCitOnCommission,
          marginPercent: inputs[`${item.id}_marginPercent`] !== "" ? parseNumInput(inputs[`${item.id}_marginPercent`]) : null,
          marginOverrideUsd: parseNumInput(inputs[`${item.id}_marginOverrideUsd`]),
          targetDdpPriceUsd: parseNumInput(inputs[`${item.id}_targetDdpPriceUsd`]),
          customValues: {},
        }));
        ...
        const res = calculateCBU(engineItems, {
          ...
          targetMarginPercent: parseNumInput(inputs.targetMarginPercent),
          cbuMode: inputs.cbuMode as any,
          customColumns: [],
        });
        setCbuResult(res);
      } catch (e: any) {
        console.error("CBU calculation error:", e);
        setRenderError(e.message || "Lỗi tính toán CBU.");
      }
    }
    ...
    try {
      const res = await fetch(`/api/rfq/${rfqId}/calculate-cbu`, {
        ...
        body: JSON.stringify({
          ...
          daysPerYear: parseNumInput(inputs.daysPerYear),
          targetMarginPercent: parseNumInput(inputs.targetMarginPercent),
          cbuMode: inputs.cbuMode,
          commissionRate: parseNumInput(inputs.commissionRate),
          citOnCommission: parseNumInput(inputs.citOnCommission),
          customColumns: [],
  ...
  if (renderError) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-3">
          <p className="text-red-600 font-medium text-lg">⚠️ Đã xảy ra lỗi trong quá trình tính toán.</p>
          <p className="text-slate-600 max-w-md">{renderError}</p>
          <button onClick={() => window.location.reload()} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 mt-4">
            Tải lại trang
          </button>
        </div>
      </div>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-500">Đang khởi tạo bảng tính...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="sm:max-w-[1400px] w-[95vw] max-h-[90vh] flex flex-col p-0 gap-0 rounded-2xl overflow-hidden border border-slate-200/80 shadow-2xl bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-5 bg-slate-50/50 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* PRICING PARAMETERS (theo Excel: F3-F13) */}
            <div className="bg-white border border-slate-200 rounded-lg p-6 space-y-5 lg:col-span-2">
              <h3 className="font-semibold text-slate-800 border-b pb-3">PRICING PARAMETERS <span className="text-xs font-normal text-slate-400 ml-2">(ô vàng = input)</span></h3>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <div className="flex flex-col gap-1.5">
                  <label className={`text-sm font-semibold ${inputs.cbuMode === "PRICE_INPUT" ? "text-slate-400" : "text-blue-600"}`}>Target margin (m) %</label>
                  <input type="text" inputMode="decimal" value={inputs.targetMarginPercent} onChange={(e) => handleInput("targetMarginPercent", e.target.value)} disabled={inputs.cbuMode === "PRICE_INPUT"} className={GLOBAL_INPUT_CLASS + " text-right font-mono font-bold " + (inputs.cbuMode === "PRICE_INPUT" ? "opacity-50 bg-slate-50 cursor-not-allowed" : "!border-blue-400 focus:!ring-blue-500")} />
                  <span className="text-[10px] text-slate-500">Lãi gộp trên giá bán (Global Fallback)</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-blue-600">Commission rate (q) %</label>
                  <input type="text" inputMode="decimal" value={inputs.commissionRate} onChange={(e) => handleInput("commissionRate", e.target.value)} className={GLOBAL_INPUT_CLASS + " text-right font-mono !border-blue-400 focus:!ring-blue-500 font-bold"} />
                  <span className="text-[10px] text-slate-500">Tính trên giá bán DDP USD</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-blue-600">CIT on commission (c) %</label>
                  <input type="text" inputMode="decimal" value={inputs.citOnCommission} onChange={(e) => handleInput("citOnCommission", e.target.value)} className={GLOBAL_INPUT_CLASS + " text-right font-mono !border-blue-400 focus:!ring-blue-500 font-bold"} />
                  <span className="text-[10px] text-slate-500">Thuế nhà thầu trên hoa hồng</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-700">% Value financed</label>
                  ...
                  <span className="text-[10px] text-slate-500">Theo điều khoản thanh toán</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pt-6 border-t border-slate-100">
                <div className="flex flex-col gap-1.5">
                  ...
                  <span className="text-[10px] text-slate-500">Chi phí vốn</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-700">Financing days</label>
                  <input type="text" inputMode="decimal" value={inputs.financingDays} onChange={(e) => handleInput("financingDays", e.target.value)} className={GLOBAL_INPUT_CLASS + " text-right font-mono"} />
                  <span className="text-[10px] text-slate-500">Số ngày tài trợ vốn</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-700">Days per year</label>
                  <input type="text" inputMode="decimal" value={inputs.daysPerYear} onChange={(e) => handleInput("daysPerYear", e.target.value)} className={GLOBAL_INPUT_CLASS + " text-right font-mono"} />
                  <span className="text-[10px] text-slate-500">Cơ sở quy đổi lãi</span>
                </div>
              </div>
          ...
          {/* ── MARGIN ANALYSIS TABLE (khớp Excel) ────────────────── */}
          <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
            ...
            <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between flex-wrap gap-3">
              <div>
                <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider">MARGIN ANALYSIS – TÍNH GIÁ DDP</h2>
                <p className="text-[11px] text-slate-500 mt-0.5">Chữ <span className="text-blue-600 font-semibold">XANH DƯƠNG</span> = ô nhập tay · chữ <span className="text-slate-900 font-semibold">ĐEN</span> = công thức</p>
              </div>
              ...
              <div className="flex gap-5 items-center">
                ...
                <div className="w-px h-7 bg-slate-200" />
                <div className="text-right">
                  <p className="text-[10px] text-blue-500 font-bold uppercase tracking-wider">Total Revenue</p>
                  <p className="text-sm font-bold text-blue-700 font-mono">${fmt(cbuResult.totalRevenueUsd)}</p>
                </div>
                <div className="w-px h-7 bg-slate-200" />
                <div className="text-right">
                  <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-wider">Nominal Margin %</p>
                  <p className={`text-sm font-bold font-mono ${cbuResult.nominalMarginPct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                    {fmt(cbuResult.nominalMarginPct, 2)}%
                  </p>
                </div>
                <div className="w-px h-7 bg-slate-200" />
                <div className="text-right">
                  ...
                  <p className={`text-sm font-bold font-mono ${cbuResult.effectiveMarginPct >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                    {fmt(cbuResult.effectiveMarginPct, 2)}% (${fmt(cbuResult.effectiveGrossProfitUsd)})
                  </p>
                </div>
              </div>
            </div>
              ...
              <table className="text-[11px] border-collapse min-w-max w-full">
                <thead className="sticky top-0 z-10 shadow-sm">
                  <tr className="bg-slate-100 border-b-2 border-slate-200 divide-x divide-slate-200">
                    <th className="text-left px-2 py-2 text-slate-600 font-bold uppercase tracking-wider w-[40px]">ITEM</th>
                    <th className="text-left px-2 py-2 text-slate-600 font-bold uppercase tracking-wider min-w-[120px]">Part No.</th>
                    <th className="text-left px-2 py-2 text-slate-600 font-bold uppercase tracking-wider min-w-[150px]">Description</th>
                    <th className="text-center px-2 py-2 text-slate-600 font-bold uppercase tracking-wider w-[55px]">Q'ty</th>
                    <th className="text-right px-2 py-2 text-blue-600 font-bold uppercase tracking-wider min-w-[80px]">Wt (lbs)</th>
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[70px]">Wt (kg)</th>
                    <th className="text-right px-2 py-2 text-blue-600 font-bold uppercase tracking-wider min-w-[90px]">Material Cost</th>
                    <th className="text-right px-2 py-2 text-blue-600 font-bold uppercase tracking-wider min-w-[60px]">%Duty</th>

                    {/* Computed cost columns */}
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[75px]">Bank fee</th>
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[75px]">Logistics</th>
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[70px]">Duty</th>
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[80px]">Commission</th>
                    <th className="text-right px-2 py-2 text-slate-500 font-bold uppercase tracking-wider min-w-[55px]">CIT</th>

                    {/* Unit Cost */}
                    <th className="text-right px-2 py-2 text-slate-900 font-bold uppercase tracking-wider min-w-[90px] bg-slate-200/60">Unit Cost</th>

                    {/* Margin overrides (input) */}
                    {inputs.cbuMode !== "PRICE_INPUT" && (
                      <>
                        <th className="text-right px-2 py-2 text-blue-700 font-bold uppercase tracking-wider min-w-[85px] bg-blue-50">Margin %</th>
                        <th className="text-right px-2 py-2 text-blue-700 font-bold uppercase tracking-wider min-w-[95px] bg-blue-50">Margin $</th>
                      </>
                    )}

                    {/* DDP Price */}
                    <th className="text-right px-2 py-2 text-blue-700 font-bold uppercase tracking-wider min-w-[90px] bg-blue-50">Sale Price (USD)</th>
                    <th className="text-right px-2 py-2 text-violet-700 font-bold uppercase tracking-wider min-w-[100px] bg-violet-50">Sale Price (VND)</th>
                    <th className="text-right px-2 py-2 text-violet-700 font-bold uppercase tracking-wider min-w-[110px] bg-violet-50">Total Revenue (VND)</th>

                    {/* Margin results */}
                    <th className="text-right px-2 py-2 text-emerald-700 font-bold uppercase tracking-wider min-w-[90px] bg-emerald-50">Margin per unit</th>
                    <th className="text-right px-2 py-2 text-emerald-700 font-bold uppercase tracking-wider min-w-[70px] bg-emerald-50">% Margin</th>
                    <th className="text-right px-2 py-2 text-emerald-700 font-bold uppercase tracking-wider min-w-[90px] bg-emerald-50">Total Margin</th>
                    <th className="text-right px-2 py-2 text-slate-600 font-bold uppercase tracking-wider min-w-[90px] bg-slate-50">Total Cost</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(cbuResult?.items || []).map((item, idx) => {
                    ...
                    const totalCostLine = item.totalCostUsd || 0;
                    const totalMarginLine = item.totalMarginUsd || 0;
                    const marginPerUnit = item.marginPerUnitUsd || 0;
                    const pctMargin = item.marginPercentActual || 0;
                    const weightKg = item.extWeightKg || 0;
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-blue-50/30 transition-colors divide-x divide-slate-100">
                        ...
                        {/* Total Weight (lbs) — INPUT */}
                        <td className="px-2 py-1.5">
                          <input type="text" inputMode="decimal" placeholder="0" value={inputs[`${item.id}_netWeightLbs`] ?? ""} onChange={(e) => handleInput(`${item.id}_netWeightLbs`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-200 focus:!border-blue-400 text-blue-700"} />
                        </td>
                        {/* Weight (kg) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(weightKg, 6)}</td>

                        {/* Material Cost — INPUT */}
                        <td className="px-2 py-1.5">
                          <input type="text" inputMode="decimal" placeholder="0" value={inputs[`${item.id}_supplierUnitPrice`] ?? ""} onChange={(e) => handleInput(`${item.id}_supplierUnitPrice`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-200 focus:!border-blue-400 text-blue-700"} />
                        </td>
                        {/* %Duty — INPUT */}
                        <td className="px-2 py-1.5">
                          <input type="text" inputMode="decimal" placeholder="0" value={inputs[`${item.id}_dutyPercent`] ?? ""} onChange={(e) => handleInput(`${item.id}_dutyPercent`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-200 focus:!border-blue-400 text-blue-700"} />
                        </td>

                        {/* Bank fee ($) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(item.bankFeePerUnit)}</td>
                        {/* Logistics ($) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(item.logisticsPerUnit)}</td>
                        {/* Duty ($) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(item.dutyPerUnit)}</td>
                        {/* Commission ($) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(item.commissionPerUnit)}</td>
                        {/* CIT ($) — computed */}
                        <td className="px-2 py-1.5 text-right text-slate-500 font-mono bg-slate-50/50">{fmt(item.citPerUnit)}</td>

                        {/* Unit Cost ($) */}
                        <td className="px-2 py-1.5 text-right text-slate-900 font-bold font-mono bg-slate-200/40 border-x-2 border-slate-300">
                        ...
                        {inputs.cbuMode !== "PRICE_INPUT" && (
                          <td className="px-2 py-1.5 bg-blue-50/40">
                            <input type="text" inputMode="decimal" placeholder={inputs.targetMarginPercent || "0"} value={inputs[`${item.id}_marginPercent`] ?? ""} onChange={(e) => handleInput(`${item.id}_marginPercent`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-300 focus:!border-blue-600 font-bold text-blue-700"} />
                          </td>
                        )}

                        {/* Margin $/unit override — INPUT */}
                        {inputs.cbuMode !== "PRICE_INPUT" && (
                          <td className="px-2 py-1.5 bg-blue-50/40">
                            <input type="text" inputMode="decimal" placeholder="0" value={inputs[`${item.id}_marginOverrideUsd`] ?? ""} onChange={(e) => handleInput(`${item.id}_marginOverrideUsd`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-300 focus:!border-blue-600 font-bold text-blue-700"} />
                          </td>
                        )}

                        {/* DDP Price (USD) */}
                        <td className="px-2 py-1.5 text-right text-blue-700 font-bold font-mono bg-blue-50/40">
                          {inputs.cbuMode === "PRICE_INPUT" ? (
                            <input type="text" inputMode="decimal" placeholder="0" value={inputs[`${item.id}_targetDdpPriceUsd`] ?? ""} onChange={(e) => handleInput(`${item.id}_targetDdpPriceUsd`, e.target.value)} className={NUM_INPUT_CLASS + " !border-blue-300 focus:!border-blue-600 font-bold text-blue-700"} />
                          ) : (
                        ...
                        {/* DDP Price (VND) */}
                        <td className="px-2 py-1.5 text-right text-violet-700 font-bold font-mono bg-violet-50/30">{fmtVnd(item.ddpPriceVnd)}</td>

                        {/* Total Revenue (VND) */}
                        <td className="px-2 py-1.5 text-right text-violet-700 font-bold font-mono bg-violet-50/30">{fmtVnd(totalDdpVnd)}</td>

                        {/* Margin per unit ($) */}
                        <td className={`px-2 py-1.5 text-right font-bold font-mono bg-emerald-50/20 ${marginPerUnit >= 0 ? "text-emerald-700" : "text-red-600"}`}>
                          {fmt(marginPerUnit)}
                        </td>

                        {/* % Margin */}
                        <td className={`px-2 py-1.5 text-right font-bold font-mono bg-emerald-50/20 ${pctMargin >= 0 ? "text-emerald-700" : "text-red-600"}`}>
                          {fmt(pctMargin, 2)}%
                        </td>

                        {/* Total Margin ($) */}
                        <td className={`px-2 py-1.5 text-right font-bold font-mono bg-emerald-50/20 ${totalMarginLine >= 0 ? "text-emerald-700" : "text-red-600"}`}>
                          {fmt(totalMarginLine)}
                        </td>

                        {/* Total Cost ($) */}
                        <td className="px-2 py-1.5 text-right text-slate-600 font-mono bg-slate-50/50">{fmt(totalCostLine)}</td>
                      </tr>
                    );
                  })}
                </tbody>
