...
export default function CBUCalcPage() {
  ...
  const handleParseQuote = async () => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        showToast("Bóc tách báo giá Hãng bằng AI thành công!", "ok");
        await fetchRFQ();
        setQuoteFile(null);
      } else {
        showToast(result.error || "Có lỗi khi bóc tách.", "err");
      }
    } catch (e) {
      showToast("Có lỗi khi gọi API bóc tách.", "err");
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      {/* ── SECTION 1.5: Supplier Quote Test Zone ───────────────────────────── */}
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 border border-slate-700 rounded-2xl p-6 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              TẢI BÁO GIÁ HÃNG (SUPPLIER QUOTE TEST ZONE)
            </h2>
            <p className="text-slate-300 text-sm mt-1">
              Kéo thả file báo giá (PDF/XLSX) từ Hãng vào đây để AI bóc tách Mã Quote & Giá nhập tự động vào bảng bên dưới.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative group flex-1 md:flex-none">
              ...
              />
              <div className={`px-4 py-2.5 rounded-xl border-2 border-dashed flex items-center justify-center gap-2 transition-all ${quoteFile ? 'border-blue-400 bg-blue-500/10' : 'border-slate-500 bg-slate-800 hover:border-slate-400 hover:bg-slate-700'}`}>
                <svg className={`w-5 h-5 ${quoteFile ? 'text-blue-400' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                <span className={`text-sm font-medium truncate max-w-[200px] ${quoteFile ? 'text-blue-300' : 'text-slate-300'}`}>
                  {quoteFile ? quoteFile.name : "Chọn hoặc Kéo thả file..."}
                </span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={!quoteFile || parsingQuote}
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:hover:bg-blue-600 text-white font-semibold text-sm transition-colors flex items-center gap-2 whitespace-nowrap shadow-lg shadow-blue-900/50"
            >
              {parsingQuote ? (
                <>
                  ...
                  Gemini đang đọc file {quoteFile?.name}...
                </>
              ) : (
                <>
